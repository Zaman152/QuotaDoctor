...
/* ─── AI employees data (from Vendasta, adapted for QuotaDoctor) ─── */
const aiTeam = [
  {
    id: "receptionist",
...
const setupServices = [
  {
    ...
    desc: "Expert configuration so your AI Receptionist learns your business, handles calls perfectly, and starts booking appointments from day one.",
    icon: "🚀",
  },
  {
    title: "AI Reputation Setup",
    desc: "We train the AI to respond to reviews in your brand voice and auto-trigger review requests after every completed job.",
    icon: "⭐",
  },
  {
    title: "AI Data Analyst Setup",
    desc: "We connect your data sources, CRM, and review platforms so your AI Analyst delivers weekly insight reports automatically.",
    icon: "📊",
  },
  {
    title: "AI Sales Setup",
    desc: "Configure AI to qualify leads, handle objections, and book meetings directly into your calendar without manual work.",
    icon: "💼",
  },
  {
    title: "AI Support Setup",
    desc: "We train your support agent on your knowledge base so it resolves customer issues 24/7 using your exact processes.",
    icon: "🛟",
  },
];
...
function ParallaxHero() {
  ...
  return (
    ...
    >
      ...
      {/* Kinetic marquee background text */}
      ...
      <motion.div style={{ y, opacity }} className="relative z-10 text-center px-4 max-w-5xl mx-auto">
        <motion.span
          ...
          initial="hidden"
          animate="visible"
          className="inline-block text-xs font-bold tracking-[0.3em] uppercase text-[#4AACDE] bg-white/50 backdrop-blur-md border border-white/60 px-5 py-2 rounded-full mb-8"
        >
        ...
        >
          YOUR AI
          <br />
          <em className="font-playfair font-medium italic text-[#4AACDE] not-italic" style={{ fontStyle: "italic" }}>
            Workforce
          </em>
          <br />
          NEVER SLEEPS.
        ...
        >
          <a
            ...
            className="glass-card-dark inline-flex items-center gap-3 bg-[#0A2F4C] text-white px-10 py-5 rounded-full font-black text-lg shadow-2xl hover:scale-105 transition-transform"
          >
...
function StatsRow() {
  return (
    <section className="py-20 bg-[#0A2F4C] relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-8 relative z-10">
        {stats.map((s, i) => (
          ...
          >
            <p className="font-playfair text-[clamp(2.5rem,6vw,5rem)] font-bold text-white leading-none mb-2" style={{ fontFamily: "var(--font-playfair), Georgia, serif" }}>
              {s.value}
            </p>
...
/* ─── Main AI Team Section ─── */
function AITeamSection() {
  return (
    <section id="ai-team" className="py-32 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-20">
          ...
          <motion.h2
            ...
            whileInView="visible"
            viewport={{ once: true, margin: "-80px" }}
            className="text-[clamp(3rem,7vw,6rem)] font-black text-[#0A2F4C] leading-[0.88] tracking-tighter"
          >
            MEET YOUR
            <br />
            <span className="font-playfair italic text-[#4AACDE] font-medium" style={{ fontFamily: "var(--font-playfair), Georgia, serif", fontStyle: "italic" }}>
              AI Employees.
            </span>
          </motion.h2>
          <motion.p
            ...
            whileInView="visible"
            viewport={{ once: true, margin: "-80px" }}
            className="mt-6 text-xl text-[#6B7280] max-w-2xl mx-auto"
          >
          ...
          {aiTeam.map((emp, idx) => (
            <motion.div
              ...
              className={`glass-card rounded-[2.5rem] p-8 relative group overflow-hidden flex flex-col ${
                idx === 0 || idx === 6 ? "md:col-span-2 lg:col-span-1" : ""
              }`}
            >
              {/* Glow blob */}
              <div
                className="absolute -top-16 -right-16 w-48 h-48 rounded-full blur-[80px] opacity-25 group-hover:opacity-40 group-hover:scale-125 transition-all duration-700 pointer-events-none"
              ...
              {emp.badge && (
                <span
                  className="absolute top-6 right-6 text-xs font-black px-3 py-1 rounded-full text-white"
                  style={{ background: emp.accent }}
                >
                  {emp.badge}
                </span>
              ...
              {/* Icon */}
              <div className="text-5xl mb-6">{emp.icon}</div>

              {/* Stat pill */}
              <div
                className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-white font-black text-sm mb-5 w-fit"
                style={{ background: `rgba(${emp.accentRgb}, 0.15)`, color: emp.accent, border: `1px solid rgba(${emp.accentRgb}, 0.3)` }}
              >
                <span className="text-xl font-black">{emp.stat}</span>
                <span className="font-medium text-xs opacity-80">{emp.statLabel}</span>
              </div>
              ...
              <p className="text-sm font-bold text-[#4AACDE] mb-4 uppercase tracking-wider">
                {emp.tagline}
              </p>
              <p className="text-[#6B7280] leading-relaxed flex-1">{emp.description}</p>
...
function HorizontalShowcase() {
  return (
    <section className="py-24 overflow-hidden bg-[#F7F9FB]">
      <div className="max-w-7xl mx-auto px-4 mb-12">
        <motion.h2
          ...
          whileInView="visible"
          viewport={{ once: true, margin: "-80px" }}
          className="text-[clamp(2rem,5vw,4rem)] font-black text-[#0A2F4C] tracking-tighter text-center"
        >
          THE COMPLETE{" "}
          <span className="font-playfair italic font-medium text-[#4AACDE]" style={{ fontFamily: "var(--font-playfair), Georgia, serif", fontStyle: "italic" }}>
            Customer Journey
          </span>
        </motion.h2>
        <motion.p
      ...
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          >
            <div
              className="absolute top-0 left-0 right-0 h-1 rounded-t-[2.5rem] opacity-80"
              style={{ background: col.color }}
            />
            <div
              className="absolute -bottom-16 -right-16 w-48 h-48 rounded-full blur-[80px] opacity-10 group-hover:opacity-20 transition-opacity duration-700 pointer-events-none"
              style={{ background: col.color }}
            />
            <p className="font-black tracking-[0.2em] text-xs uppercase mb-4" style={{ color: col.color }}>
              {col.phase}
            </p>
            <h3 className="text-2xl font-black text-[#0A2F4C] tracking-tight mb-6">{col.title}</h3>
            <ul className="space-y-3">
              {col.features.map((f) => (
                <li key={f} className="flex items-start gap-3 text-[#6B7280] text-sm leading-relaxed">
                  <span className="w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center mt-0.5" style={{ background: `${col.color}20`, color: col.color }}>
                    ✓
                  </span>
                  {f}
                </li>
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            >
              ...
              <div className="mt-8">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-sm text-[#4AACDE] font-bold hover:text-white transition-colors flex items-center gap-2 group/link"
                >
                  Get started <span className="group-hover/link:translate-x-1 transition-transform">→</span>
                </a>
              </div>
            </motion.div>
          ))}
          ...
          <motion.div
            ...
            whileHover={{ y: -6, scale: 1.02 }}
            viewport={{ once: true, margin: "-80px" }}
            className="rounded-[2rem] p-8 relative group overflow-hidden flex flex-col justify-between"
            style={{
              background: "rgba(74,172,222,0.15)",
              border: "1px solid rgba(74,172,222,0.3)",
              backdropFilter: "blur(20px)",
            }}
          >
            <div>
              <div className="text-4xl mb-5">✨</div>
              <h3 className="text-xl font-black text-white mb-3">Not sure where to start?</h3>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="mt-8 w-full flex items-center justify-center gap-2 bg-[#4AACDE] text-white font-black py-4 rounded-2xl text-sm hover:bg-[#3A9CCC] transition-colors"
            >
              Book Free Call →
            </a>
          </motion.div>
...
function Testimonial() {
  return (
    <section className="py-28 relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 right-0 w-[40vw] h-[40vw] bg-[#4AACDE]/10 rounded-full blur-[120px]" />
      </div>
      <div className="max-w-4xl mx-auto px-4 relative z-10 text-center">
        <motion.div
          ...
          whileInView="visible"
          viewport={{ once: true, margin: "-80px" }}
          className="glass-card rounded-[3rem] p-12 md:p-20"
        >
          <div className="text-7xl mb-8 opacity-20 font-black text-[#0A2F4C]">"</div>
          <blockquote
            className="text-2xl md:text-3xl text-[#0A2F4C] leading-relaxed mb-10 font-playfair italic"
          ...
          <div className="flex flex-col items-center gap-1">
            <div className="w-12 h-12 rounded-full bg-[#0A2F4C] flex items-center justify-center text-white font-black text-lg mb-2">
              MK
            </div>
            <p className="font-black text-[#0A2F4C]">Michael K.</p>
            <p className="text-[#6B7280] text-sm">Business Owner using QuotaDoctor AI</p>
          </div>
        </motion.div>
      </div>
    </section>
